...
import { FetchError, createFetch, ofetch } from "ofetch";
...
//#endregion
//#region src/chrome/chrome-api.ts
var CwsApi = class {
	constructor(options) {
	...
	uploadEndpoint(extensionId) {
		return new URL(`https://www.googleapis.com/upload/chromewebstore/v1.1/items/${extensionId}`);
	}
	publishEndpoint(extensionId) {
		return new URL(`https://www.googleapis.com/chromewebstore/v1.1/items/${extensionId}/publish`);
	}
	...
	async submitForReview(params) {
		...
		if (params.publishTarget) endpoint.searchParams.append("publishTarget", params.publishTarget);
		if (params.deployPercentage != null) endpoint.searchParams.set("deployPercentage", String(params.deployPercentage));
		if (params.reviewExemption != null) endpoint.searchParams.set("reviewExemption", String(params.reviewExemption));
		await fetch(endpoint.href, {
			method: "POST",
			headers: {
				Authorization,
	...
	getToken() {
		return fetch(this.tokenEndpoint().href, {
			method: "POST",
			body: JSON.stringify({
...
//#endregion
//#region src/chrome/chrome-web-store.ts
const ChromeWebStoreOptions = z.object({
	zip: z.string().min(1),
...
var EdgeApi = class {
	...
	*/
	async uploadDraft(params) {
		...
		if (!operationId) throw Error("Edge API did not return an operation ID in the Location header.");
		return { operationId };
	}
	/**
	...
	*/
	async publish(params) {
		await fetch(`https://api.addons.microsoftedge.microsoft.com/v1/products/${params.productId}/submissions`, {
			method: "POST",
			body: JSON.stringify({}),
			...
			throw new Error(`Edge API failed to publish: ${err.status} ${err.statusText} - ${data}`);
		});
	}
...
//#endregion
//#region src/edge/edge-addon-store.ts
const EdgeAddonStoreOptions = z.object({
	zip: z.string().min(1),
...
var EdgeAddonStore = class {
	...
	async submit(dryRun) {
		...
		await withTimeout(this.uploadAndPollValidation(token, 5e3), 10 * 6e4);
		if (this.options.skipSubmitReview) {
			this.setStatus("Skipping submission (skipSubmitReview=true)");
			return;
		}
	...
	async uploadAndPollValidation(token, pollIntervalMs) {
		...
		else this.setStatus("Extension is valid");
	}
};

//#endregion
//#region src/firefox/firefox-api.ts
var AddonsApi = class {
	constructor(options) {
		this.options = options;
	}
	addonDetailEndpoint(extensionId) {
		return new URL(`https://addons.mozilla.org/api/v5/addons/addon/${extensionId}`);
	}
	...
	addonsUploadDetailsEndpoint(uploadUuid) {
		return new URL(`https://addons.mozilla.org/api/v5/addons/upload/${uploadUuid}`);
	}
...
//#endregion
//#region src/firefox/firefox-addon-store.ts
const FirefoxAddonStoreOptions = z.object({
	zip: z.string().min(1),
...
var FirefoxAddonStore = class {
	...
	async submit(dryRun) {
		...
		const addon = await this.api.details({ extensionId: this.extensionId });
		if (dryRun) {
			this.setStatus("DRY RUN: Skipped upload and publishing");
			return;
		}
		...
		this.setStatus(`Validation results: ${plural(errors, "error")}, ${plural(warnings, "warning")}, ${plural(notices, "notice")}`);
...
function validateConfig(config) {
	...
	if (!result.success) throw Error("Missing required config: " + result.error.issues.map((i) => i.path.map((j) => toScreamingSnakeCase(String(j))).join("_")).join(", "), { cause: result.error });
	return result.data;
}
...
async function submit(config) {
	...
	if (internalConfig.dryRun) consola.warn("Dry run, skipping submission");
	if (internalConfig.edge?.clientSecret || internalConfig.edge?.accessTokenUrl) consola.warn([
	...
	const results = {};
	await new Listr(stores.map(({ id, name, getStore }) => ({
		title: name,
	...
	if (errors.length > 0) throw Error(`Submissions failed: ${errors.length}`, { cause: errors });
	return results;
}
...
async function init(config) {
	...
	const stores = await prompt("What stores do you want to configure? \x1B[2m(use ↑/↓ and space to select)\x1B[0m", {
		type: "multiselect",
		options: [
			{
				value: "chrome",
...
async function prompt(message, options, previousValue) {
	...
	if (typeof result === "symbol") throw Error("Canceled");
...
async function initChrome(previousOptions) {
	...
	const extensionId = await prompt("Enter the extension ID:", { type: "text" }, previousOptions?.extensionId);
	entries.push(["CHROME_EXTENSION_ID", extensionId]);
	console.log();
	consola.log("`--chrome-client-id` and `--chrome-client-secret` are generated by following the \"Initial Setup\" from:");
	console.log("https://developer.chrome.com/docs/webstore/using-api#setup");
	const clientId = await prompt("Enter your client ID:", { type: "text" }, previousOptions?.clientId);
	...
	if (await prompt("Generate new refresh token?", { type: "confirm" })) {
		...
		const authCode = await consola.prompt("Open the above URL, login, and enter the auth code:", {
			type: "text",
			required: true
		});
	...
	const publishTarget = await prompt("`--chrome-publish-target`: Where do you want to release to?", {
		type: "select",
		options: [{
			label: "Default",
			value: "default",
	...
	const submitForReview = await prompt("When uploading, automatically submit new update for review?", { type: "confirm" }, String(!previousOptions?.skipSubmitReview));
...
async function initFirefox(previousOptions) {
	...
	consola.info("Your `--firefox-extension-id` is listed at the bottom of the details page on:");
	console.log("https://addons.mozilla.org/en-US/developers/");
	const extensionId = await prompt("Enter extension ID:", { type: "text" }, previousOptions?.extensionId);
	entries.push(["FIREFOX_EXTENSION_ID", extensionId]);
	console.log();
	consola.log("`--firefox-jwt-issuer` and `--firefox-jwt-secret` are available at:");
	console.log("https://addons.mozilla.org/developers/addon/api/key/");
	const jwtIssuer = await prompt("Enter your JWT issuer:", { type: "text" }, previousOptions?.jwtIssuer);
	entries.push(["FIREFOX_JWT_ISSUER", jwtIssuer]);
	const jwtSecret = await prompt("Enter your JWT secret:", { type: "text" }, previousOptions?.jwtSecret);
	entries.push(["FIREFOX_JWT_SECRET", jwtSecret]);
	const channel = await prompt("`--firefox-channel`: Which channel do you want to release to?", {
		type: "select",
		options: [{
			label: "Listed",
			value: "listed",
...
async function initEdge(previousOptions) {
	...
	consola.info("Your `--edge-product-id` is listed On the developer dashboard, at the top of the page under the extension name");
	console.log("https://partner.microsoft.com/dashboard/microsoftedge/overview");
	const productId = await prompt("Enter product ID:", { type: "text" }, previousOptions?.productId);
	entries.push(["EDGE_PRODUCT_ID", productId]);
	console.log();
	consola.log("`--edge-client-id` and either `--edge-api-key` (API v1.1) or `--edge-client-secret` and `--edge-access-token-url` (API v1.0) can be created following:");
	console.log("https://learn.microsoft.com/en-us/microsoft-edge/extensions-chromium/publish/api/using-addons-api#before-you-begin");
	const clientId = await prompt("Enter your client ID:", { type: "text" }, previousOptions?.clientId);
	entries.push(["EDGE_CLIENT_ID", clientId]);
	const apiKey = await prompt("Enter your API key:", { type: "text" }, previousOptions?.apiKey);
...
async function updateEnvFile(entries) {
	...
	for (const [name, value] of entries) {
		const replacement = `${name}=${typeof value === "string" ? `"${value}"` : value}`;
...
//#endregion
export { FirefoxAddonStoreOptions as a, EdgeAddonStoreOptions as c, ChromeWebStoreOptions as d, CwsApi as f, FirefoxAddonStore as i, EdgeApi as l, submit as n, AddonsApi as o, InlineConfig as r, EdgeAddonStore as s, init as t, ChromeWebStore as u };